...
'''
LA IDEA ES VER COMO SE NOS OFRECEN LOS DATOS DEL CSV PARA SABER COMO TRATARLOS
'''

'''
CREAMOS EL DATAFRAME DE CSV_BICIMAD Y LO 
TRATAMOS EN FUNCIÓN DE CÓMO NOS HA SALIDO EN EL CÓDIGO ANTERIOR
sep = '\t'
'''
...
for index, estacion in csv_bicimad.iterrows():
    # Verifica los criterios especificados
    if estacion['light'] != 3 and estacion['activate'] == 1 and estacion['no_available'] == 0 and estacion['free_bases'] != 0:
        # Calcula la distancia geodésica entre el monumento y la estación
        ...
        # Agrega la estación candidata a la lista
        estaciones_candidatas.append({'Nombre': estacion['name'], 'Distancia': distancia})

# Ordena la lista de estaciones candidatas por distancia
estaciones_candidatas = sorted(estaciones_candidatas, key=lambda x: x['Distancia'])

# Encuentra la estación más cercana que cumpla con los criterios
